...
const METRIC_CONFIG := [
	...
	{"key": "stance", "label": "STANCE", "icon": "stance", "fill": "stance"},
...
func _render() -> void:
	...
	for region in LIMB_ORDER:
		...
		if trauma != "NONE":
			active_traumas += 1
		_render_limb(name, current, maximum, trauma, state)

	if _structure_status:
		_structure_status.text = "STRUCTURE %d/7 STABLE // %d TRAUMA" % [
			healthy_regions,
			active_traumas,
		]
		HUDAssetLibrary.apply_label(
			_structure_status,
			"critical" if active_traumas > 0 else "body"
		)
...
func _render_limb(
	...
	trauma_label.text = (
		"%s / %s // %s"
		% [_compact(current), _compact(maximum), trauma.replace("_", " ")]
	)
	HUDAssetLibrary.apply_label(
		trauma_label,
		"critical" if trauma != "NONE" else "muted"
	)
