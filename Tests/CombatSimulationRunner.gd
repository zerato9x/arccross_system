...
func _run_simulation() -> void:
	...
	for i in range(total_battles):
		...
		battle_player.inventory.paper_doll.clear()
		...
		if duel_outcome == -999:
			...
			print("Battle ", i + 1, " finished with outcome: ", GameEnums.CombatOutcome.keys()[duel_outcome])
			success_count += 1
