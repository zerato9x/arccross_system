extends Node2D
## Demo: HexRenderer con texture_fn y animation_fn.
## Muestra un grid de 10x10 usando PlaceholderTexture2D por terreno.
## Reemplazar PlaceholderTexture2D con Texture2D/AtlasTexture reales en producción.
...
func _texture_fn(cell: HexCell) -> Texture2D:
	# PlaceholderTexture2D muestra un patrón de tablero.
	# En producción: load("res://assets/tiles/%s.png" % terrain_name)
	var tex := PlaceholderTexture2D.new()
	tex.size = Vector2(HexGrid.HEX_SIZE, HexGrid.HEX_SIZE)
	return tex
